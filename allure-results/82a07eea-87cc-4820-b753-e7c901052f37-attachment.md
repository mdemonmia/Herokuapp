...
- Name: ContactlistPage.spec.js >> Contact list page test >> check logout button to redirect the back page
- Location: tests\ContactlistPage.spec.js:263:10
...
Received: "https://thinking-tester-contact-list.herokuapp.com/logout"

Call log:
  - Expect "toHaveURL" with timeout 5000ms
    3 × unexpected value "https://thinking-tester-contact-list.herokuapp.com/logout"
...
```ts
  ...
  261 |     })
  262 | 
  263 |     test.only('check logout button to redirect the back page',async({page})=>{
  264 |         const contact = new contactlistPage(page);
  265 |         await contact.clickAddContactBtn();
  266 |         await contact.clicklogoutBtn();
> 267 |         expect(page).toHaveURL('https://thinking-tester-contact-list.herokuapp.com/');
      |                      ^ Error: expect(page).toHaveURL(expected) failed
  268 |     })
  269 | })
```