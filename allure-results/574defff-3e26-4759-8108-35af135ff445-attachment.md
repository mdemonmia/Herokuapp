...
- generic [active] [ref=e1]:
  - generic [ref=e2]:
    ...
    - table [ref=e10]:
      ...
      - rowgroup
      - row "Dilruba Akter 2000-05-28 dilrubaakterdiparpicse04@gmail.com 01738438619 Uttara,Dhaka Sector-11 Dhaka Uttara 1206 Bangladesh" [ref=e20]:
        - cell "Dilruba Akter" [ref=e21]
        - cell "2000-05-28" [ref=e22]
        - cell "dilrubaakterdiparpicse04@gmail.com" [ref=e23]
...
```ts
  ...
  57 |      );
> 58 |      await page.waitForSelector('#edit-contact');
     |                 ^ Error: page.waitForSelector: Test timeout of 60000ms exceeded.
  59 | 
  60 | 
  61 |      //step-4 edit contact
  62 |      const contactdetails = new contactdetailsPage(page);
  63 |      await contactdetails.clickEditContactBtn();
  64 |      await contactdetails.getEditContact(data.updateContact);
  65 |      const firstName = await contactdetails.getfirstName();
  66 |      await expect(firstName).toContain(data.updateContact.firstname);
  67 |      console.log('Contact updated successfully.');
  68 | 
  69 |      //step-5 delete contact
  70 |      await contactdetails.clickDeleteContactBtn();
  71 |      await expect(page).toHaveURL('https://thinking-tester-contact-list.herokuapp.com/contactList');
  72 |      console.log('Contact deleted successfully.');
  73 | 
  74 |      //step-6 logout
  75 |      await contact.clicklogoutBtn();
  76 |      await expect(page).toHaveURL('https://thinking-tester-contact-list.herokuapp.com/logout');
  77 |      console.log('Logout successfully.');
  78 | 
  79 | })
```